...
import { ResponseFormat, type BacklogIssue, type BacklogProject } from "../types.js";
...
function toArray(val: number | number[] | undefined): number[] | undefined {
...
function formatIssue(issue: BacklogIssue): string {
  ...
  if (issue.customFields.length > 0) {
    ...
    for (const cf of issue.customFields) {
      const val =
        cf.value !== null && cf.value !== undefined ? String(cf.value) : "—";
...
Args:
  ...
  - response_format: 'markdown' (default) or 'json'`,
      inputSchema: z.object({
        ...
        status_id: numOrArray.describe(
          "Filter by status ID(s). Pass a single number (e.g. 1) or array (e.g. [1,2]). Use backlog_get_project_config to get valid IDs."
        ),
        assignee_id: numOrArray.describe(
          "Filter by assignee user ID(s). Pass a single number or array."
        ),
    ...
    async ({
      ...
      try {
        const params: Record<string, unknown> = { count: limit, offset, order };

        // Resolve project key → numeric project ID (Backlog issues API only accepts projectId[])
        if (project_key) {
          const project = await apiGet<BacklogProject>(`/projects/${project_key}`);
        ...
        const issues = await apiGet<BacklogIssue[]>("/issues", params);
        ...
        for (const issue of issues) {
          ...
          lines.push(
            `Status: **${issue.status.name}** | Priority: ${issue.priority.name} | Assignee: ${issue.assignee?.name ?? "—"} | Due: ${formatDate(issue.dueDate)}`
          );
          lines.push("");
        }
  ...
  - response_format: 'markdown' (default) or 'json'`,
    ...
    async ({
      ...
      try {
        ...
        if (estimated_hours !== undefined) body["estimatedHours"] = estimated_hours;
        ...
        const issue = await apiPost<BacklogIssue>("/issues", body);
  ...
  - response_format: 'markdown' (default) or 'json'`,
      inputSchema: z.object({
        ...
        preview: z
          ...
          .default(false)
          .describe(
            "If true, fetches current issue and shows a before/after diff WITHOUT applying changes. " +
      ...
      try {
        ...
        if (preview) {
          ...
          lines.push(
            ...
            "## Action Required",
            "Please confirm:",
            "1. Is the **assignee** correct? If not, specify who to assign to.",
            "2. Are there any other fields to update? (comment, resolution, due date, etc.)",
            "3. Once confirmed, call `backlog_update_issue` again with `preview: false` to apply."
          );
        ...
        if (status_id !== undefined) body["statusId"] = status_id;
        if (assignee_id !== undefined) body["assigneeId"] = assignee_id;
        if (issue_type_id !== undefined) body["issueTypeId"] = issue_type_id;
        if (priority_id !== undefined) body["priorityId"] = priority_id;
        if (resolution_id !== undefined) body["resolutionId"] = resolution_id;
        if (start_date !== undefined) body["startDate"] = start_date;
        if (due_date !== undefined) body["dueDate"] = due_date;
        if (estimated_hours !== undefined) body["estimatedHours"] = estimated_hours;
        if (actual_hours !== undefined) body["actualHours"] = actual_hours;
