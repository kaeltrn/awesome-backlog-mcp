...
import { ResponseFormat, type BacklogNotification } from "../types.js";
...
Returns: List of notifications with reason, project, issue, and comment info`,
    ...
    async ({ count_only, already_read, limit, offset, response_format }) => {
      try {
        if (count_only) {
          const result = await apiGet<{ count: number }>("/notifications/count");
          return {
        ...
        if (already_read !== undefined) params["alreadyRead"] = already_read;

        const notifications = await apiGet<BacklogNotification[]>("/notifications", params);
