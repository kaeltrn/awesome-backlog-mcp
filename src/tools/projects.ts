    ...
    async ({ project_id_or_key, archived, response_format }) => {
      try {
        if (project_id_or_key !== undefined) {
          const project = await apiGet<BacklogProject>(
            `/projects/${project_id_or_key}`
          );
        ...
        const projects = await apiGet<BacklogProject[]>("/projects", params);
  ...
  - customFields includes: id, name, typeId, required, items (for list-type fields)`,
    ...
    async ({ project_key, response_format }) => {
      try {
        ...
        const [statuses, priorities, resolutions, issueTypes, categories, versions, customFields] =
          await Promise.all([
            apiGet<BacklogStatus[]>(`/projects/${key}/statuses`),
            apiGet<BacklogPriority[]>("/priorities"),
            apiGet<BacklogResolution[]>("/resolutions"),
            apiGet<BacklogIssueType[]>(`/projects/${key}/issueTypes`),
            apiGet<BacklogCategory[]>(`/projects/${key}/categories`),
            apiGet<BacklogVersion[]>(`/projects/${key}/versions`),
            apiGet<BacklogCustomField[]>(`/projects/${key}/customFields`),
          ]);
        ...
        lines.push("## Statuses");
        for (const s of statuses) lines.push(`- **${s.name}** (id: ${s.id})`);
        lines.push("");

        lines.push("## Issue Types");
        for (const t of issueTypes) lines.push(`- **${t.name}** (id: ${t.id})`);
        lines.push("");

        lines.push("## Priorities");
        for (const p of priorities) lines.push(`- **${p.name}** (id: ${p.id})`);
        lines.push("");

        lines.push("## Resolutions");
        for (const r of resolutions) lines.push(`- **${r.name}** (id: ${r.id})`);
        lines.push("");
