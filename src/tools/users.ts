...
import { ResponseFormat, type BacklogUser } from "../types.js";

function formatUser(user: BacklogUser): string {
  return [
...
Args:
  - response_format: 'markdown' (default) or 'json'`,
    ...
    async ({ response_format }) => {
      try {
        const user = await apiGet<BacklogUser>("/users/myself");
    ...
    async ({ user_id, project_key, response_format }) => {
      try {
        if (user_id !== undefined) {
          const user = await apiGet<BacklogUser>(`/users/${user_id}`);
        ...
        const users = await apiGet<BacklogUser[]>(endpoint);
