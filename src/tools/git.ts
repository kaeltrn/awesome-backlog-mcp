...
import { ResponseFormat, type BacklogPullRequest, type BacklogPullRequestComment, type BacklogRepository } from "../types.js";

function formatRepo(repo: BacklogRepository): string {
  return [
...
function formatPRComment(comment: BacklogPullRequestComment): string {
  return [
    `### Comment #${comment.id}`,
...
function formatPR(pr: BacklogPullRequest): string {
  return [
    ...
    async ({ project_id_or_key, repo_id_or_name, response_format }) => {
      try {
        if (repo_id_or_name !== undefined) {
          const repo = await apiGet<BacklogRepository>(
            `/projects/${project_id_or_key}/git/repositories/${repo_id_or_name}`
          );
        ...
        const repos = await apiGet<BacklogRepository[]>(
          `/projects/${project_id_or_key}/git/repositories`
        );
    ...
    async ({
      ...
      try {
        ...
        if (pr_number !== undefined) {
          const pr = await apiGet<BacklogPullRequest>(`${basePath}/${pr_number}`);
        ...
        const params: Record<string, unknown> = { count: limit, offset };
        if (status_id?.length) {
        ...
        const prs = await apiGet<BacklogPullRequest[]>(basePath, params);
        ...
        for (const pr of prs) {
          ...
          lines.push(
            `Status: **${pr.status.name}** | Branch: \`${pr.branch}\` → \`${pr.base}\` | By: ${pr.createdUser.name}`
          );
          lines.push("");
        }
...
Args:
  ...
  - response_format: 'markdown' (default) or 'json'`,
    ...
    async ({
      ...
      try {
        const body: Record<string, unknown> = { summary, description, base, branch };
        if (assignee_id !== undefined) body["assigneeId"] = assignee_id;
        if (issue_id !== undefined) body["issueId"] = issue_id;

        const pr = await apiPost<BacklogPullRequest>(
          `/projects/${project_id_or_key}/git/repositories/${repo_id_or_name}/pullRequests`,
          body
        );
...
Args:
  ...
  - response_format: 'markdown' (default) or 'json'`,
    ...
    async ({
      ...
      try {
        ...
        const pr = await apiPatch<BacklogPullRequest>(
          `/projects/${project_id_or_key}/git/repositories/${repo_id_or_name}/pullRequests/${pr_number}`,
          body
        );
  ...
  server.registerTool(
    "backlog_get_pull_request_comments",
    {
  ...
  - response_format: 'markdown' (default) or 'json'`,
    ...
    async ({ project_id_or_key, repo_id_or_name, pr_number, count, min_id, max_id, order, response_format }) => {
      try {
        ...
        const comments = await apiGet<BacklogPullRequestComment[]>(
          `/projects/${project_id_or_key}/git/repositories/${repo_id_or_name}/pullRequests/${pr_number}/comments`,
          params
        );
...
Returns: The created comment with ID, author, and timestamp`,
    ...
    async ({ project_id_or_key, repo_id_or_name, pr_number, content, response_format }) => {
      try {
        const comment = await apiPost<BacklogPullRequestComment>(
          `/projects/${project_id_or_key}/git/repositories/${repo_id_or_name}/pullRequests/${pr_number}/comments`,
          { content }
        );
