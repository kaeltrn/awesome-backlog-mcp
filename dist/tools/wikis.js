...
export function registerWikiTools(server) {
    server.registerTool("backlog_get_wikis", {
  ...
  - Single mode: Full page including content`,
        ...
        try {
            if (wiki_id !== undefined) {
                const wiki = await apiGet(`/wikis/${wiki_id}`);
            ...
            const params = { projectIdOrKey: project_id_or_key };
            if (keyword)
                params["keyword"] = keyword;
            const wikis = await apiGet("/wikis", params);
            if (response_format === ResponseFormat.JSON) {
    ...
    server.registerTool("backlog_create_wiki", {
...
Args:
  ...
  - response_format: 'markdown' (default) or 'json'`,
        ...
        try {
            const wiki = await apiPost("/wikis", {
    ...
    server.registerTool("backlog_update_wiki", {
...
Args:
  ...
  - response_format: 'markdown' (default) or 'json'`,
        ...
        try {
            ...
            if (mail_notify !== undefined)
                body["mailNotify"] = mail_notify;
            const wiki = await apiPatch(`/wikis/${wiki_id}`, body);
            if (response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: jsonOutput(wiki) }] };
            }
            return {
